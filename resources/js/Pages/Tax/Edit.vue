...
const form = useForm({
    min_amount: props?.tax?.min_amount ?? "",
    max_amount: props?.tax?.max_amount ?? "",
    rate_percentage: props?.tax?.rate_percentage ?? "",
    effective_year: props?.tax?.effective_year ?? "",
});

function create_tax() {
    form.put(route("taxbreacket.update",props.tax.id), {
        onSuccess: (success) => {
            console.log(success);
        },
        onError: (err) => {
...
<template>
    <div class="p-6">
        ...
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-gray-500">Update tax bracket</p>
        </div>
